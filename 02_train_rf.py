...
ext = '_nordkit' if not use_rdkit_features else '_rdkit'

input_data_fn= ''.join([data_type,ext ,'.csv'])
input_data_file = os.path.join(args.dataset_path,data_type,'processed',input_data_fn)
...
with open(os.path.join(args.dataset_path,'train_test_idxs.pickle'), 'rb') as handle:
