...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.dataset == "az" or self.dataset == "dy" or self.dataset == "su": ###az, dy, su
            smiles_list, rdkit_mol_objs, labels = \
                _load_yields_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                ## convert aromatic bonds to double bonds
...
def _load_yields_dataset(input_path):
    """
    ...
    smiles_list = input_df['0_reactant'] + "." + input_df['1_reactant'] + "." + input_df['2_reactant']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    # tasks = ['NR-AR', 'NR-AR-LBD', 'NR-AhR', 'NR-Aromatase', 'NR-ER', 'NR-ER-LBD',
    #    'NR-PPAR-gamma', 'SR-ARE', 'SR-ATAD5', 'SR-HSE', 'SR-MMP', 'SR-p53']
    labels = input_df[['yield']]
    # convert 0 to -1
