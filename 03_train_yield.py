...
parser.add_argument("-ud","--use_domain", type=str, required='True', help="use domain features or not. options: rdkit: combination od rdkit feature and bozhao features. no_rdkit: only bozhao features. False: neither.")
...
if args.use_domain=='no_rdkit':
    with_domain='w_domain'
elif args.use_domain=='rdkit':
    with_domain='rdkit_w_domain'
else:
...
'''
su_dataset = RxnGD('su_test.json', path=args.dataset_path)
dy_dataset = RxnGD('dy_test.json', path=args.dataset_path)
az_dataset = RxnGD('az_test.json', path=args.dataset_path)
'''
...
#charge_size, nmr_size = sample['charge_feats'].shape[-1], sample['nmr_feats'].shape[-1]
...
train_r2.close() ;
train_l.close();
test_r2.close();valid_r2.close();test_l.close();valid_l.close()
