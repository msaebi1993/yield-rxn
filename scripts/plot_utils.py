...
def plot_actual_vs_predicted(model_results,fig_path,key,alpha):
    ...
    plt.savefig(fig_path+key+'_actual_vs_predicted.png',dpi=300)
    plt.show()
...
def plot_fig_2(train_scores,test_scores,ylim,loss_score,model_path):
    plt.style.use('seaborn-whitegrid')
    fig, ax = plt.subplots(figsize=(6,4))
    
    ax.plot(train_scores, label="Train curve")
    ax.plot(test_scores, label="Test curve")
    
    ax.set_xlabel('Number of epochs',fontsize=15)
    ax.set_ylabel(loss_score,fontsize=15)
    #ax.set_ylim([0,1])
