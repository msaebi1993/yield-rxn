...
strings = []
bond_changes = []
for line in open("./data/tmp.txt.proc", "r"):
    string = line.split()[0]
...
for string in strings:
    ...
    rct_mols_map = [[atom.GetIntProp('molAtomMapNumber') for atom in mol.GetAtoms()] for mol in rct_mols]
    prod_mols_map = [[atom.GetIntProp('molAtomMapNumber') for atom in mol.GetAtoms()] for mol in prod_mols]
    ...
    for i, mol in enumerate(rct_mols):
        in_prods = False
        rct_atom_maps = set(rct_mols_map[i])
        curr_rct = set(rct_smiles[i])
        for j, mol in enumerate(prod_mols):
            prod_atom_maps = set(prod_mols_map[j])
            curr_prod = set(prod_smiles[j])
            if (rct_atom_maps & prod_atom_maps):
                in_prods = True
                break
